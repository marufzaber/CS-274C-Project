...
def train(num_epochs, batch_size, learning_rate, output_dir):
    ...
    model.add(Reshape([640, 480*4]))
    model.add(Conv1D(input_shape=shape, filters=8, kernel_size=3, activation="relu"))
    model.add(BatchNormalization())
    model.add(Reshape([638, 8]))

    model.add(MaxPooling1D(pool_size=2))
    model.add(Reshape([319, 8]))

    model.add(Conv1D(filters=8, kernel_size=2, activation="relu"))
    model.add(BatchNormalization())
    model.add(Reshape([318, 8]))

    model.add(MaxPooling1D(pool_size=2))
    model.add(Reshape([159, 8]))

    model.add(Conv1D(filters=8, kernel_size=2, activation="relu"))
    model.add(BatchNormalization())
    model.add(Reshape([158, 8]))

    model.add(MaxPooling1D(pool_size=2))
    model.add(Reshape([79, 8]))
